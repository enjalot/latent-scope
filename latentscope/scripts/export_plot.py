...
def main():
    ...
    parser.add_argument(
        "dataset_id", type=str, help="Dataset id (directory name in data folder)"
    )
    parser.add_argument("scope_id", type=str, help="Scope id")
    parser.add_argument(
        "--plot_config", type=str, default=None, help="Optional plot config"
    )
...
def dmp(dataset_id, scope_id, plot_config=None):
    ...
    scope_parquet = pd.read_parquet(
        os.path.join(DATA_DIR, dataset_id, "scopes", scope_id + ".parquet")
    )
    ...
    interactive_specific_options = {"point_radius_max_pixels": 4}
    is_interactive = "interactive" in plot_config and plot_config["interactive"]
    del plot_config["interactive"]
    ...
    if is_interactive:
        ...
        for key in keys_to_delete:
            if key in plot_config:
                del plot_config[key]
        # add_glow
        del plot_config["add_glow"]
        plot = datamapplot.create_interactive_plot(
            xy_columns,
            simplified_labels,
            enable_search=True,
            use_medoids=True,
            point_radius_max_pixels=interactive_specific_options["point_radius_max_pixels"],
            **plot_config,
        )
        plot_path = os.path.join(directory, f"{id}.html")
        plot.save(plot_path)
