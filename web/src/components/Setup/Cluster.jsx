...
function Cluster() {
  ...
  // then collapsed when user-generated clusterings already exist
  ...
  useEffect(() => {
    ...
    const scopeCluster = scope?.cluster_id ? clusters?.find((c) => c.id == scope.cluster_id) : null;
    if (scopeCluster) {
  ...
  useEffect(() => {
    if (dataset) {
      ...
      apiService.fetchClusters(dataset?.id).then((cls) => {
        setClusters(cls);
        setFormOpen((open) => (open === null ? cls.length === 0 : open));
      });
    }
  }, [dataset]);
  ...
  const formExpanded = formOpen === true || !!clusterJob;
        ...
        >
          <div className={styles['cluster-form']}>
                ...
                <>
                  <label>
                    ...
                    <Tooltip id="n_neighbors" place="top" effect="solid" className="tooltip-area">
                      Number of neighbors for the kNN graph. Higher values capture broader
                      structure.
                    </Tooltip>
                  </label>
                  <label>
                ...
                <>
                  ...
                  <label>
                    ...
                    <Tooltip
                      ...
                      className="tooltip-area"
                    >
                      The cluster selection epsilon parameter sets a distance threshold that allows
                      you to balance the density of clusters. Set to 0 to use pure HDBSCAN.
                    </Tooltip>
                  </label>
                </>
              ) : null}
            ...
            <JobProgress
              ...
              killJob={(job) =>
                apiService.killJob(dataset.id, job.id).then(setClusterJob).catch(console.error)
              }
            />
          </div>
        </CreationPanel>
        ...
        <div className={styles['cluster-list']}>
          ...
          {umap && clusters.filter((c) => c.umap_id === umap?.id).length >= 2 && (
            <div className={styles['compare-link']}>
              <Link to={`/datasets/${dataset?.id}/compare-clusters/`}>↗ Compare Clusters</Link>
            </div>
          )}
        </div>
      </div>
