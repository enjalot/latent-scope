...
function Scope() {
  ...
  const { startJob: startScopeJob } = useStartJobPolling(
    dataset,
    setScopeJob,
    `${apiUrl}/jobs/scope`
  );
  const { startJob: startDeleteScopeJob } = useStartJobPolling(
    dataset,
    setScopeJob,
    `${apiUrl}/jobs/delete/scope`
  );
  ...
  useEffect(() => {
    if (dataset && scope?.cluster_id) {
      apiService
        .fetchClusterLabelsAvailable(dataset?.id, scope?.cluster_id)
        .then((cls) => setClusterLabelSets(cls));
    }
  }, [dataset, scope]);
  ...
  useEffect(() => {
    console.log('setting label', embedding, clusterLabelSet, scope);
    if (embedding && clusterLabelSet && !scope.label) {
      setLabel(embedding.model_id?.replace('___', '/') + ' + ' + clusterLabelSet.model_id);
    }
  ...
  useEffect(() => {
    ...
    if (
      lsVersion &&
      savedScope?.ls_version &&
      compareVersions(savedScope?.ls_version, lsVersion) < 0
    ) {
      setNewVersion(true);
    }
        ...
        <div className={styles['scope-form']}>
          <form onSubmit={handleSaveScope}>
            <label>
              ...
              <input
                type="text"
                name="label"
                defaultValue={scope ? scope.label : label}
                value={label}
            ...
            <label>
              ...
              <input
                type="text"
                name="description"
                defaultValue={scope ? scope.description : ''}
          ...
          <div className={styles['scope-setup-info']}>
            ...
            <span className={savedScope?.embedding_id !== embedding?.id ? styles['different'] : ''}>
              ...
              <span className={styles['scope-form-value']}>
                {embedding?.id} - {embedding?.model_id}
              </span>
              <br />
            </span>
            {scope?.sae_id ? (
            ...
            <span className={savedScope?.cluster_id !== cluster?.id ? styles['different'] : ''}>
              <span className={styles['scope-form-label']}>Cluster: </span>
              <span className={styles['scope-form-value']}>
                {cluster?.id} - ({cluster?.n_clusters} clusters)
              </span>
              <br />
            </span>
            <span
              className={
                savedScope?.cluster_labels_id !== clusterLabelSet?.id ? styles['different'] : ''
              }
            >
              <span className={styles['scope-form-label']}>Labels: </span>
              <span className={styles['scope-form-value']}>
                {clusterLabelSet?.id} - {clusterLabelSet?.model_id}
              </span>
              <br />
            </span>
            <span className={savedScope?.ls_version !== lsVersion ? styles['different'] : ''}>
              <span className={styles['scope-form-label']}>Version: </span>
              <span className={styles['scope-form-value']}>{lsVersion}</span>
              <br />
            </span>
          </div>
          {(savedScope && isDifferent) || newVersion ? (
            <div className={styles['previous-scope']}>
              <h4>Previous Scope Settings</h4>
              <span
                className={savedScope.embedding_id !== embedding?.id ? styles['different'] : ''}
              >
                <span className={styles['scope-form-label']}>Embedding: </span>
                <span className={styles['scope-form-value']}>
                  {savedScope.embedding_id} - {savedEmbedding?.model_id}
                </span>
                <br />
              </span>
              <span className={savedScope.umap_id !== umap?.id ? styles['different'] : ''}>
                <span className={styles['scope-form-label']}>Umap: </span>
                <span className={styles['scope-form-value']}>{savedScope.umap_id}</span>
                <br />
              </span>
              <span className={savedScope.cluster_id !== cluster?.id ? styles['different'] : ''}>
                <span className={styles['scope-form-label']}>Cluster: </span>
                <span className={styles['scope-form-value']}>
                  {savedScope.cluster_id} - ({savedCluster?.n_clusters} clusters)
                </span>
                <br />
              </span>
              <span
              ...
              >
                <span className={styles['scope-form-label']}>Labels: </span>
                <span className={styles['scope-form-value']}>
                  {savedScope.cluster_labels_id} - {savedClusterLabelSet?.model_id || 'N/A'}
                </span>
                <br />
              </span>
              <span className={savedScope.ls_version !== lsVersion ? styles['different'] : ''}>
                <span className={styles['scope-form-label']}>Version: </span>
                <span className={styles['scope-form-value']}>{savedScope.ls_version}</span>
                <br />
              </span>
            </div>
          ) : null}
